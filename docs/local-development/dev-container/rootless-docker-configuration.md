...
| FWS_API_HOST_USERNAME | Username for the **host** account used when creating a dev container. This **must** be the account running rootless Docker and have entries in /etc/subuid and /etc/subgid accordingly. |

## Run Configuration Shell Script

[setup-for-rootless-docker-with-dev-container.sh](../../../docker/scripts/setup-for-rootless-docker-with-dev-container.sh) **must** be run as **root** before attempting to create a dev container from either a local fws-api repository or by cloning the remote fws-api repository into a container volume. This script validates the mandatory environment variables and exits if configuration issues are detected. If no configuration issues are detected, the following actions are performed regardless of how dev containers are created:

* Rootful Docker socket backup.
* Replacement of rootful Docker socket with a symbolic link to the rootless Docker socket.
  * This is required because the Docker socket used by LocalStack and [Docker outside Of Docker Compose](https://github.com/devcontainers/templates/tree/main/src/docker-outside-of-docker-compose) dev containers does not appear to be configurable at the time of writing.
...
volumes to be created succesfully. Ownership of this directory is **transferred** to the host SUBUID and SUBGID for the dev container **vscode** user in the
[post dev container creation script](../../../docker/scripts/post-create-command.sh).
