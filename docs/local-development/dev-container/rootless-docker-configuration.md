...
[Ensure that a local fws-api repository for use with a development container exists on the host](./local-repository-creation.md)
...
| FWS_API_HOST_USERNAME | Username for the **host** account used when creating a development container. This **must** be the account running rootless Docker and have entries in /etc/subuid and /etc/subgid accordingly. |
...
To facilitate development container creation from the local fws-api repository, ownership of the local fws-api repository is **transferred** to the host SUBUID and SUBGID for the development container **vscode** user.

* This action forces work to be performed within the development container rather than using the local fws-api repository directly.
...
When creating a development container by cloning the remote fws-api repository into a container volume, the development container user has ownership of items in the volume without risk of git reporting dubious ownership.
...
```sh
sudo --preserve-env=LOCAL_FWS_API_DIR,FWS_API_HOST_USERNAME "$LOCAL_FWS_API_DIR"/docker/scripts/setup-for-rootless-docker-with-dev-container.sh
```
