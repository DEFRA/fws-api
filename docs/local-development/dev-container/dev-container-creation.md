...
* When using macOS or non-systemd enabled WSL 2 with Docker Desktop the dev container **must** be created using a
...
* When using native Linux or systemd enabled WSL 2 with native Docker the dev container can be created from either a local fws-api repository on the development machine or a remote repository URL.
  * When using WSL 2 the [\\wsl drive](https://learn.microsoft.com/en-us/windows/wsl/filesystems#interoperability-between-windows-and-linux-commands) can be used to access the local fws-api respository from Visual Studio Code on Windows.
