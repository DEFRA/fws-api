...
lab.experiment('Notification API Client', () => {
  ...
  lab.test(' 4 - getting 400 response code when severity is not 1-4', async () => {
    ...
    Code.expect(response.statusCode).to.equal(400)
    Code.expect(response.messageLog.errorMessage).to.equal('error')
  })
  ...
  lab.test(' 6 - getting 400 response code when language is not English', async () => {
    const response = await handler({
    ...
    Code.expect(response.statusCode).to.equal(400)
    Code.expect(response.messageLog.errorMessage).to.equal('Failed warning message - wrong language')
  })
  lab.test(' 7 - getting 400 response code when database is unavailible', async () => {
    sinon.stub(services.prototype, 'getLastMessage').callsFake(() => {
      return Promise.resolve({
        rows: [{
          id: '153',
