...
class Message {
  ...
  async save (xmlResult) {
    ...
    // if last message exists and severity hasnt changed set situation/severity changed to last message values
    if (message.lastMessage !== undefined && JSON.stringify(message.severityValue) === message.lastMessage.severity_value) {
      message.situationChanged = message.lastMessage.situation_changed
