...
module.exports.handler = async (event, context) => {
  ...
  try {
    ...
    const lambda = new AWS.Lambda({
      endpoint: new AWS.Endpoint(process.env.FWS_LAMBDA_ENDPOINT || 'localhost')
    })

    const params = {
      FunctionName: (process.env.FWS_ENV_NAME || 'dev') + 'fws-putWarningsAlerts',
      Payload: '',
    ...
    const lambdaInvokeResult = await lambda.invoke(params).promise()
    console.log('DEBUG: lambdaInvokeResult: ', lambdaInvokeResult)
