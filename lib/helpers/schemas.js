/**
 *Schema used to generate and validate message model object
 */
'use strict'
...
const validateMessage = Joi.object({
  ...
  situation: Joi.string().max(990).required().when('severityValue', { is: Joi.number().valid(5), then: Joi.allow('') }),
